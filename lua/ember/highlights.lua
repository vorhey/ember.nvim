...
local function hi_bg(name, opts)
...
function M.apply(p)
	...
	hi("FloatBorder", { fg = p.violet, bg = p.base2 })
	...
	hi("LineNr", { fg = p.base8 })
	hi("CursorLineNr", { fg = p.accent, bold = true })
	hi("SignColumn", { fg = p.base7, bg = p.bg })
	hi("FoldColumn", { fg = p.base7, bg = p.bg })
	hi("Folded", { fg = p.base6, bg = p.base2 })
	hi("VertSplit", { fg = p.base5 })
	hi("WinSeparator", { fg = p.base5 })
	...
	hi_bg("PmenuSel", { fg = p.accent, bg = p.base4, bold = true })
	hi("PmenuSbar", { bg = p.base3 })
	hi("PmenuThumb", { bg = p.base5 })

	-- ── Search / Selection ──
	hi_bg("Visual", { fg = p.white, bg = p.visual_bg })
	hi_bg("VisualNOS", { bg = p.base5 })
	hi_bg("Search", { fg = p.search_fg, bg = p.search_bg, bold = true })
	hi_bg("IncSearch", { fg = p.search_fg, bg = p.search_bg, bold = true })
	hi_bg("CurSearch", { fg = p.search_fg, bg = p.search_bg, bold = true })
	hi("Substitute", { fg = p.bg, bg = p.red })

	-- ── Flash ──
	hi_bg("FlashLabel", { fg = p.white, bg = p.melon, bold = true })
	hi_bg("FlashMatch", { fg = p.search_fg, bg = p.search_bg })
	hi_bg("FlashCurrent", { fg = p.search_fg, bg = p.search_bg, bold = true })
	hi("FlashBackdrop", { fg = p.muted })
	hi("FlashPrompt", { fg = p.accent })
	hi("LspReferenceText", { sp = p.accent, underline = true })
	hi("LspReferenceRead", { sp = p.accent, underline = true })
	hi("LspReferenceWrite", { sp = p.accent, underline = true, bold = true })
	hi("IlluminatedWordText", { sp = p.accent, underline = true })
	hi("IlluminatedWordRead", { sp = p.accent, underline = true })
	hi("IlluminatedWordWrite", { sp = p.accent, underline = true, bold = true })
	...
	hi("NonText", { fg = p.base7 })
	hi("SpecialKey", { fg = p.base7 })
	hi("Conceal", { fg = p.base7 })
	hi("MatchParen", { fg = p.accent, bg = p.base4, bold = true })
	hi("Whitespace", { fg = p.base6 })

	-- ── Diff ──
	hi_bg("DiffAdd", { fg = p.success, bg = p.diff_add_bg })
	hi_bg("DiffChange", { bg = p.diff_change_bg })
	hi_bg("DiffDelete", { fg = p.error, bg = p.diff_del_bg })
	hi_bg("DiffText", { bg = p.diff_text_bg, bold = true })
	...
	hi_bg("SnacksPickerSelected", { fg = p.accent, bg = p.base3, bold = true })
	...
	hi("TelescopeBorder", { fg = p.violet, bg = p.bg })
	hi("TelescopeNormal", { fg = p.fg, bg = p.bg })
	hi_bg("TelescopeSelection", { fg = p.accent, bg = p.base3, bold = true })
	...
	hi("SnacksNotifierBorder", { fg = p.violet, bg = p.base2 })

	-- ── Snacks terminal ──
	hi("SnacksTerminalNormal", { fg = p.fg, bg = p.bg })
	hi("SnacksTerminalBorder", { fg = p.violet, bg = p.bg })

	-- ── Snacks diff ──
	hi_bg("SnacksDiffAdd", { fg = p.success, bg = p.diff_add_bg })
	hi_bg("SnacksDiffDelete", { fg = p.error, bg = p.diff_del_bg })
	hi_bg("SnacksDiffContext", { bg = p.bg })
	hi_bg("SnacksDiffHeader", { fg = p.blue, bg = p.base3, bold = true })
	hi("SnacksDiffHunkHeader", { fg = p.base7, italic = true })
	...
	hi("WhichKeyBorder", { fg = p.violet, bg = p.bg })
	...
	hi("SnacksDashboardFooter", { fg = p.base7 })
	...
	hi_bg("BlinkCmpMenuSelection", { fg = p.accent, bg = p.base4, bold = true })
	hi("BlinkCmpMenuBorder", { fg = p.violet, bg = p.base2 })
	...
	hi("BlinkCmpDocBorder", { fg = p.violet, bg = p.base2 })
	...
	hi("NeoTreeFloatBorder", { fg = p.violet, bg = sb_bg })
	...
	-- ── Mini.files ──
	hi("MiniFilesBorder", { fg = p.violet, bg = p.base2 })
